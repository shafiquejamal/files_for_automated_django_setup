...
echo " "
echo "----------------------------------------------------------------"
echo "1. On the HOST (in order to run the django development server on the host), create database with this info:"
echo "databases_default_name:$databases_default_name"
echo "databases_default_user:$databases_default_user"
echo "databases_default_password:$databases_default_password"
echo " "
...
echo " "
echo "----------------------------------------------------------------"
echo "2. On the HOST (in order to run the django development server on the host), synchronize and migrate the database:"
...
echo " 0.  $ cd $1_repo 											# on the host, to access the guest machine"
echo " A1. $ vagrant up 											# on the host, to access the guest machine"
echo " A2. $ vagrant ssh 											# on the host, to access the guest machine"
echo " B. $ mkvirtualenv $1_venv 								# on the guest"
echo " C. $ pip install -r /vagrant/requirements/local.txt			# on the guest"
echo " E. $ sudo -u postgres psql #Create the database, role, and grant priviliges, then \q to quit  # on the guest"
echo "	CREATE DATABASE $databases_default_name;"
echo "	CREATE USER $databases_default_user WITH PASSWORD '$databases_default_password';"
echo "	GRANT ALL PRIVILEGES ON DATABASE $databases_default_name to $databases_default_user;"
echo "	\l"
